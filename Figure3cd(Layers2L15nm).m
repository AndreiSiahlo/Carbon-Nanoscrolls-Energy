Print["  Figure3cd(Layers2L30nmRIn2d1nm.m"];
Print[];
...
Print[" dPhi12 - The difference of the inner angles of the \
spirales"];
Print["   of the Layers"];
dPhi12eq0 = 0.0 Pi;
dPhi12eqPi = 1.0 Pi;
...
iEVdW = 1; iEVdW1Un1 = 2; iEVdW1Ov1 = 3; iEVdW1Un2 = 4; iEVdW1Ov2 = \
5;
...
Print[" II.2.1. The function fSpiraleLen[", NoLp, 
  ",\[CurlyPhi]In, \[CurlyPhi]Out, h] defines"];
Print["  the Length of a Spirale with the inner agle \[CurlyPhi]In \
and the outer angle \[CurlyPhi]Out(>=\[CurlyPhi]In):"];
fSpiraleLen[NoLv_, PhiInv_, PhiOutv_, hv_] :=
   (1/(4 Pi) hv NoLv (-PhiInv Sqrt[1 + PhiInv^2] + 
             PhiOutv Sqrt[1 + PhiOutv^2] - ArcSinh[PhiInv] + 
             ArcSinh[PhiOutv]));
Print["  fSpiraleLen[", NoLp, ", \[CurlyPhi]In, \[CurlyPhi]Out, h]=", 
  fSpiraleLen[NoLp, PhiIn, PhiOut, h], "."];
Print[" II.2.2. The function fElast[\[CurlyPhi]In,\[Rho]Out] is \
required to calculate an nanoscrollelastic energy: "];
fElast[PhiInv_, 
...
Print[" fElast[\[CurlyPhi]In,\[CurlyPhi]Out] = ", 
  fElast[PhiIn, PhiOut], "."];
Print[" II.2.3. Define the function fPhiOutvsPhiInLh[", NoLp, ",", \
PhiIn, ",L,h]."]
...
Print[" The function fPhiOutvsPhiInLh[", NoLp, ",", PhiIn, ",L,h]=", 
  fPhiOutvsPhiInLh[NoLp, PhiIn, L, h], " is a
   good approximation to obtain the value of \[CurlyPhi]Out for the \
defined \[CurlyPhi]In,L,h."];
fPhiInvsPhiOutLh[NoLv_, PhiOutv_, Lv_, hv_] := 
    Sqrt[PhiOutv^2 - 4 \[Pi] Lv/(NoLv hv)];
Print[" The inverse function fPhiInvsPhiOutLh[", NoLp, 
  ",\[CurlyPhi]Out, L, h]]=", 
   fPhiInvsPhiOutLh[NoLp, PhiOut, L, h]];
Print[" could be used in the program applications if ROut (instead of \
RIn) is the input parameter of the system."];

Print[" II.2.4. The functions \
fSpirale1Under(Over)Spirale1Length[NoLv,PhiIn1v ,PhiOut1v,hv]" ];
fSpirale1UnderSpirale1Length[NoLv_, PhiIn1v_ , PhiOut1v_, hv_] := 
  fSpiraleLen[NoLv, PhiIn1v , PhiOut1v - 2 Pi, hv];
fSpirale1OverSpirale1Length[NoLv_, PhiIn1v_ , PhiOut1v_, hv_] := 
  fSpiraleLen[NoLv, PhiIn1v + 2 Pi , PhiOut1v, hv];

fSpirale1UnderSpirale2Length[NoLv_, PhiIn1v_ , PhiOut1v_, hv_, 
   dPhi12v_] := 
  fSpiraleLen[NoLv, PhiIn1v, PhiOut1v - 2 Pi/NoLv, hv]; 
fSpirale1OverSpirale2Length[NoLv_, PhiIn1v_ , PhiOut1v_, hv_, 
   dPhi12v_] := 
  fSpiraleLen[NoLv, PhiIn1v + 2 Pi/NoLv + dPhi12v, PhiOut1v, hv];
fSpirale2UnderSpirale1Length[NoLv_, PhiIn1v_ , PhiOut1v_, hv_, 
   dPhi12v_] := 
  fSpiraleLen[NoLv, PhiIn1v, PhiOut1v - 2 Pi/NoLv, hv]; 
fSpirale2OverSpirale1Length[NoLv_, PhiIn1v_ , PhiOut1v_, hv_, 
   dPhi12v_] := 
  fSpiraleLen[NoLv, PhiIn1v - dPhi12v + 2 Pi/NoLv, PhiOut1v - dPhi12v,
    hv];

Print["  These functiona are not required, dut could be helpful),"];

If[NoLp == 1,
  Print["fSpirale1UnderSpirale1Length[1,PhiIn1v ,PhiOut1v,hv]="];
  Print["  =fSpiraleLen[NoLv,PhiIn1v ,PhiOut1v-2Pi,hv]=", 
   fSpiraleLen[NoLv, PhiIn1v , PhiOut1v - 2 Pi, hv], ";"];
  Print["  fSpirale1UnderSpirale1Length[NoLp,PhiIn1p \
,PhiOut1p,hp]=fSpirale1UnderSpirale1Length[", NoLp, ",", 
  ...
  Print["  =fSpiraleLen[NoLv,PhiIn1v+2Pi ,PhiOut1v,hv]=", 
   fSpiraleLen[NoLv, PhiIn1v + 2 Pi , PhiOut1v, hv], ";"];
  Print["  fSpirale1OverSpirale1Length[NoLp,PhiIn1p \
,PhiOut1p,hp]=fSpirale1OverSpirale1Length[", NoLp, ",", 
   PhiIn1p/(2 Pi), "(2Pi),", PhiOut1p/(2 Pi), "(2Pi),", hp/nm, 
   "nm] ="];
  Print["  =", 
...
,PhiOut1p,hp]=fSpirale1OverSpirale1Length[", NoLp, ",", 
  ...
  Print["  fSpiraleLen[NoLv,PhiIn1v, PhiOut1v -2 Pi/NoLv,hv]=", 
   fSpirale1UnderSpirale2Length[1, PhiIn1v , PhiOut1v, hv, dPhi12v], 
   ";"];
  Print["fSpirale2UnderSpirale1Length[NoLp,PhiIn1p \
,PhiOut1p,hp,dPhi12p]=fSpirale2UnderSpirale1Length[", NoLp, ",", 
   PhiIn1p/(2 Pi), "(2Pi),", PhiOut1p/(2 Pi), "(2Pi),", hp/nm, "nm,", 
   dPhi12p/(2 Pi), "(2Pi)] ="];
  Print["  =", 
   fSpirale1UnderSpirale2Length[NoLp, PhiIn1p , PhiOut1p, hp, 
  ...
  Print["  =fSpiraleLen[NoLv,PhiIn1v+Pi ,PhiOut1v,hv]=", 
   fSpiraleLen[NoLv, PhiIn1v + Pi , PhiOut1v, hv], ";"];
  Print["fSpirale1OverSpirale2Length[NoLp,PhiIn1p \
,PhiOut1p,hp]=fSpirale1OverSpirale2Length[", NoLp, ",", 
   PhiIn1p/(2 Pi), "(2Pi),", PhiOut1p/(2 Pi), "(2Pi),", hp/nm, 
   "nm] ="];
  Print["  =", 
   fSpirale1OverSpirale2Length[NoLp, PhiIn1p , PhiOut1p, hp, dPhi12p]/
...
Print["is a good approximation to obtain the value of the sharp in \
...
If[NoLp == 1,
  Print[" III.1. The inner and the outer angle of the spirale of the \
layer:"]];
If[NoLp == 2,
  Print[" III.1. The inner and the outer angles of the spirales of \
the layers:"]];
Print[" \[CurlyPhi]In1=", RIn1 2 Pi/(NoLp h), 
  ", \[CurlyPhi]Out1=fPhiOutvsPhiInLh[", NoLp, ",\[Phi]In1,L1,h];"];
...
If[NoLp == 2, 
  Print["  \[CurlyPhi]In2=", PhiIn2dPhi12p/(2 Pi), 
   "(2Pi),\[CurlyPhi]Out2=", PhiOut2dPhi12p/(2 Pi), "(2Pi)"];
  PhiOut2dPhi12Pip = 
     fPhiOut2[NoLp, L1p, RIn1p, hp, Pi];
...
If[NoLp == 2,
  ...
  Spirale2OverSpirale1dPhi120Plot = 
     ...
     PlotRange -> {{-1.1 ROut1p/nm, 1.1 ROut1p/nm}, {-1.1 ROut1p/nm, 
                1.1 ROut1p/nm}}], {}];
  Print["Plot Spirales for dPhi12=Pi (could be NotRequired, dPhi12=0 \
in this program)"];
...
"    but could be helpful at calculation of VdW ebergy of monoscroll \
at debugging;";
"    for example,";
"    fEVdWLayer1Overlap[NoL1,Lwp,15nm, 2nm, hp, aCCp, epsVdWp] ";
" and fEVdWLayersOverlap[NoL2,Lwp,L1p=15nm, 2nm, hp, aCCp, epsVdWp, \
0]";
   ...
   {ScrollEnergyv = -10^20 eV/atom},
    ...
    ScrollEnergyv = 
     fScrollEnergyVdWandElast[NoLv, Lwv, L1v, RIn1v, hv, aCCv, 
      epsVdWv, CCv, CBNv];
    "note: the function fScrollEnergyVdWandElast[1,..] is \
analytycal";
    "whereas the function fScrollEnergy[....] uses the 'If[..]'- \
function";
    ];
    If[RIn1v/m >= fRIn1Sharp[NoLv, L1v, hv]/m,
    If[NoLv == 1, 
...
Print[" III.5. Determine the inner angles mismatch for the bi-layer \
nanoscroll 
...
Print["fScrollEnergy[NoLp,Lwp,tL1[[1]],RIn1p,hp, aCCp, epsVdWp,CCp, \
CBNp]=", fScrollEnergy[NoLp, Lwp, tL1[[1]], RIn1p, hp, aCCp, epsVdWp, 
    CCp, CBNp]/(eV/atom), " eV/atom"];
Print["fScrollEnergy[NoLp,Lwp,tL1[[1]],1nm,hp, aCCp, epsVdWp,CCp, \
CBNp]=", fScrollEnergy[NoLp, Lwp, tL1[[1]], 1 nm, hp, aCCp, epsVdWp, 
    CCp, CBNp]/(eV/atom), " eV/atom"];
...
Print["fEVdWLayer1Overlap[1,Lwp,7.nm,1.5nm,hp,aCCp,epsVdWp][[1]]=", 
  fEVdWLayer1Overlap[1, Lwp, 7. nm, 2.5 nm, hp, aCCp, 
     epsVdWp][[1]]/(eV/atom), 
  "eV/atom !=0, wrong value of the fEVdWLayer1Overlap[..] function \
because the layer does not not overlap"];
Print[];
Print["The analytical expressions of the fEVdWLayer1Overlap[..] \
function:"];
Print[];
Print["--- \
fEVdWLayer1Overlap[NoL1,Lwv,Lpv,RInv,hv,aCCv,epsVdWv][[1]]: ---"];
Print[fEVdWLayer1Overlap[NoL1, Lwv, Lpv, RInv, hv, aCCv, 
    epsVdWv][[1]]];
Print[];
Print["--- \
fEVdWLayer1Overlap[NoL2,Lwv,Lpv,RInv,hv,aCCv,epsVdWv][[1]]: ---"];
